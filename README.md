...
黒鍵の無いところ（ミとファの間・シとドの間）は上の段も空けてあるので、
キーの並びが鍵盤の見た目とそのまま重なる。右端の `@` と `[` だけは、真上の白鍵との
対応を 1 つずらしてある（厳密に合わせると `@` が「ミとファの間」に当たって黒鍵が無く、
キーが 1 つ死ぬ）。`@` がファ♯、`[` がソ♯。

US 配列で使うと右端 3 つの刻印が変わるが（`]` → `\`、`[` → `]`、`@` → `[`）、
位置は同じなので指の動きは変わらない。

左端の `A` が鍵盤の左端（画面の表示が `C4` ならド）。オクターブを動かすと鳴る音も一緒に動く。
...
| 弾く | 画面下の鍵盤。マルチタッチ（和音・連打）対応。指を滑らせるとグリッサンドになる。鍵盤の広さは 3 段階、オクターブは左右ボタンで移動。**音色はピアノ 1 種類のみ**（楽器の選択は当面出していない。理由は下の「設計上の判断」を参照） |
