...
| れんしゅう | 曲ではなく**入門の練習課題**。ドの位置・ドレミ・5 指ポジション・ひとつとばし・音階・和音・リズム・移調の 10 種と、毎回並びが変わるランダム出題 4 段階。光った鍵を押すと次へ進む「待ってくれる」方式で、間違えても進まないだけなので急かされない。和音は同時でなくてよく、全部そろえば進む。押し切ると⭐が残る |
...
- **練習課題は曲を持たず、規則から組み立てる。** 曲は誰かの作品なので、自分で弾いた
  打鍵記録であっても楽曲そのものの権利が残る。一方、音階・5 指ポジション・3 度・和音・
  リズムは「練習の型」であって特定の誰かが書いた作品ではない。データとして曲を持たず
  `src/core/exercises.ts` の規則から生成することで、**権利の問題が起きようのない形**に
  してある。移調・速度・出題のランダム化が引数だけで効くのは、その副産物。
- **課題の出力は打鍵記録と同じ `NoteEvent[]`。** だから光る鍵盤も流れるドレミも速度変更も、
  練習のために書いたコードは 1 行もない。
- **待ちながら進む練習は時計を持たない。** テンポに追われると初心者は続かない。速く弾く
  練習は既存の速度 3 段階とメトロノームが受け持つ、と役割を分けてある。
- **資格を持つ人が作る教則ではないので、運指や姿勢には踏み込まない。** 扱うのは
  「鳴らす音とタイミングを合わせる」ことだけ。教則本の曲順・課題の組み立て自体も
  著作物なので、並びは自前で組んでいる。
