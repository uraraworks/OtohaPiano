...
| おとあて | **音と音名（ドレミ）の対応を覚えるドリル。**設定を選んで「スタート」を押すと、やめるまで出題が続く（タイピング練習と同じ形）。おだいの出しかたは 💡ひかる（位置）→ 🔤もじ（音名→鍵）→ 👂おと（音→鍵）→ 🎼おんぷ（五線→鍵）の 4 通り。**はんい**（画面に出ている鍵盤の左から 3 つ／5 つ／ぜんぶ）と**いちどに**（同時に鳴らす音を 1〜3 つまで）は別々に選ぶ。答えると ◎／✗ が中央に出て、✗ のときは答えではなく「もっと 高い おと ⬆」と向きだけ教える。やりかたは「♾ずっと」と「💥1つでも まちがえたら おわり」の 2 つ。最高記録は設定ごとに残る |
