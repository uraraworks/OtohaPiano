...
- **鍵盤の音はサンプル音源ではなく Web Audio で作る**（[src/core/synth.ts](src/core/synth.ts)）。
  オルガン・ストリングス・オルゴールは倍音を足して作るが、**ピアノだけは弦の振動そのものを
  計算している**（[src/core/stringVoice.ts](src/core/stringVoice.ts)）。倍音を足す方式では、
  1 音に弦が 2〜3 本張られていて互いにずれて唸ることや、打鍵直後の複雑な音が作れず、
  どうしても電子ピアノの音になる。遅延線を一周させながら高い成分を少しずつ失わせるだけで、
  上の倍音が先に消える・打鍵音が混ざる・減衰しながら音色が丸くなる、が物理の側から出てくる。
  波形は音の高さごとに 1 回だけ計算して使い回す（1 音あたり 4〜10ms）。
