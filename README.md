...
| おとあて | **音と音名（ドレミ）の対応を覚えるドリル。**やめるまで出題が続く（タイピング練習と同じ形）。おだいの出しかたは 💡ひかる → 🔤もじ → 👂おと の 3 段階で、ヒントを 1 段ずつ外していける。範囲は ドレミ／ドレミファソ／1 オクターブ／和音入りの 4 つ。できた数・まちがえた数・音/分 が出て、最高記録が残る |
...
- **おだいは 3 段階でヒントを外す。** 光は位置を教えるだけで音名を覚えないので、いちばん下の段
  としてだけ置き、もじ（音名 → 鍵）→ おと（音 → 鍵）と上がれるようにした。
