...
子供向けピアノ＋YouTube で親子が曲を練習している体験を、タブレットのブラウザで
...
タブレットが主なので画面には出していないが、パソコンならキーボードでも弾ける。
刻印は **JIS 配列（日本語キーボード）**に合わせてある。
...
黒鍵の無いところ（シとドの間・ミとファの間）は上の段も空けてあるので、
キーの並びが鍵盤の見た目とそのまま重なる。右端の `[` だけは、真上の白鍵との
対応を 1 つずらしてある（厳密に合わせると `[` が「シとドの間」に当たって黒鍵が無く、
キーが 1 つ死ぬ）。`@` がラ♯、`[` がド♯。

US 配列で使うと右端 2 つの刻印が変わるが（`]` → `\`、`[` → `]`）、
位置は同じなので指の動きは変わらない。

左端の `A` が鍵盤の左端（画面の表示が `F3` ならファ。子供向けピアノに合わせてファ始まりにしてある）。
オクターブを動かすと鳴る音も一緒に動く。
画面の鍵盤と同じ道を通るので、打鍵の記録も「おとあて」の判定もそのまま効く。
URL を入力している間は鳴らない。
...
| 弾く | 画面下の鍵盤。マルチタッチ（和音・連打）対応。指を滑らせるとグリッサンドになる。鍵盤の広さは 3 段階、オクターブは左右ボタンで移動。左端はドではなくファ始まり（子供向けピアノに合わせた）。**音色はピアノ 1 種類のみ**（楽器の選択は当面出していない。理由は下の「設計上の判断」を参照） |
...
| おとあて | **音と音名（ドレミ）の対応を覚えるドリル。**設定を選んで「スタート」を押すと、やめるまで出題が続く（タイピング練習と同じ形）。おだいの出しかたは 💡ひかる（位置）→ 🔤もじ（音名→鍵）→ 👂おと（音→鍵）→ 🎼おんぷ（五線→鍵）の 4 通り。**はんい**（鍵盤はファ始まりだが、出題はドから数えて 3 つ／5 つ／ぜんぶ）と**いちどに**（同時に鳴らす音を 1〜3 つまで）は別々に選ぶ。答えると ◎／✗ が中央に出て、✗ のときは答えではなく「もっと 高い おと ⬆」と向きだけ教える。やりかたは「♾ずっと」と「💥1つでも まちがえたら おわり」の 2 つ。最高記録は設定ごとに残る |
...
UI は漢字＋ふりがな（HTML 標準の `<ruby>`）に統一。文言は大人向けの普通の言葉遣いで、
...
- **鍵盤は本物のグランドピアノを録音した音源を再生する**（[src/core/synth.ts](src/core/synth.ts)）。
  Salamander Grand Piano（Alexander Holm / Yamaha C5）の MP3 版を同梱。
  30 ファイル、合計 2,012,677 bytes（約 2.01 MB / 1.92 MiB）で、88 鍵を最大 ±1 半音の
  移調でカバーする。再圧縮や切り詰めはせず、録音の立ち上がり・自然減衰・ステレオを保つ。
  1 音につき 1 種類の録音で、元音源の全16段階の強弱や共鳴・ペダル専用音は含まない。
  音声を展開すると約143 MiB（44.1kHz時）を使うため、転送容量とメモリ使用量は異なる。
  「始める」で読み込み、12 秒以内に揃わない場合は表示付きで従来の合成音に切り替える。
  音源はサイト内から取得するので、静的配信・サーバー不要・API キー不要を維持できる。
  [音源のクレジットと利用条件](public/audio/piano/credits.html)はアプリの開始画面からも開ける。
- **楽器の選択は当面出していない。** ピアノ 1 種類で使う。エレクトーン・オルゴール・
  ストリングスの合成処理は残してある。過去の合成ピアノ改良（倍音ごとの減衰・弦の振動モデル）は
  採用に至らなかったため、今回は録音音源へ切り替えた。
...
- **音部記号は出さない。** 描画を 2 度作り直しても記号として通用する形にならず、文字で
  「ト音記号」と書く案も五線の脇が読みにくくなるだけだった。ドリルで問うているのは音符の
  高さの位置であって記号の判別ではないので、載せないことにした（音の位置取り自体は
  鍵盤の左端に合わせてト音・ヘ音を切り替えている）。
- **答えたら必ず ◎／✗ を通してから次へ進む。** 間を空けるだけでは「合っていたのか」が分からない
  まま次が出る。**✗ のときは答えの名前ではなく「もっと 高い／低い」と向きだけ教える。**
  名前を教えるとその場は進むが、考える機会が消える。向きだけなら自分で探して当てられるし、
  探す過程がそのまま音と鍵の対応になる（ヒントボタンはこれで不要になったので外した）。
- **難しさは 2 つの軸に分ける。** 「はんい」と「いちどに」を混ぜると、何が難しくなったのかが
  分からなくなる。「ぜんぶ」は画面に出ている鍵盤に従うので、**画面に無い鍵が答えになることはない**。
- **走っている間は設定を畳む。** おだいに縦を全部渡すため（残したままだと高さが足りず、
  五線が成績の行に重なった）。
- **縦持ちのスマホでは、飾りと余白から先に削る。** 「第◯問」の枠・最高記録の空行・鍵盤の高さを、
  縦の足りない端末でだけ詰める（詰める前は、おだいの上か下がかならず見切れていた）。
- **音のおだいは、鳴った瞬間にキャラが跳ねる。** 音だけだと画面に何も起きず、鳴ったことに
  気付けない。絵はどの音でも同じにする（絵が変われば、それが答えの手がかりになってしまう）。
  間違えたときは ✗ が消えたタイミングで自動的に鳴り直すので、再生ボタンは置いていない
  （キャラを押せば聞き直せる。まちがえの数は増えない）。
- **ドリル中の打鍵は、おだいと同じ長さだけ鳴らす。** 指を離した瞬間に切れると、
  ちょんと触れただけのときに おだいの音と聞き比べられない。
- **おとあてのタブでは動画の枠を畳む。** ドリルに動画は関係がないので、幅をそちらへ回す。
  畳む前に必ず再生を止める。**「映像を隠したまま音だけ流す」は YouTube の規約で不可**であり、
  隠した状態で再生を続けられる作りにしてはいけない。
- **正解したら少し間を置いてから次を出す。** 間が無いと、押した音と次のおだいの音が重なって
  聞き分けられない。この間の打鍵は判定しない（まだ出ていないおだいを間違い扱いにしないため）。
- **アイコンは自前の SVG**（[src/ui/icons.ts](src/ui/icons.ts)）。いろはと同じ規格
  （32×32・線は濃い紫・太さ 2・角丸・面は彩度を落とした色）。絵文字は端末ごとに絵柄も色も
  変わるので画面が揃わず、「ずっと（♾）」のように絵柄として意味が伝わらないものもある。
  HTML には `data-icon="play"` の印だけ置き、起動時に差し込む（SVG を直接書くと HTML が読めなくなる）。
...
- **鍵盤に書く音名はドレミだけ。** 今どのオクターブにいるかは道具バーの表示（F3 など）で
...
ピアノ音声（`public/audio/piano/*.mp3`）は Alexander Holm の Salamander Grand Piano、
CC BY 3.0 です。アプリの MIT ライセンスとは別に適用されます。出典・取得元の版・
ハッシュは同ディレクトリの `credits.html` と `manifest.json` に記録しています。
