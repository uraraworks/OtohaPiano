...
- **楽器の選択は当面出していない。** エレクトーン・オルゴール・ストリングスの音は用意して
  あるが、**より良い音色の作り方が決まるまで、ピアノ 1 種類で使う**という判断。
  合成音でグランドピアノらしさを出そうと、倍音ごとの減衰・弦の硬さ・打鍵音を入れた版と、
  弦の振動そのものを計算する版（撥弦モデル）を作ったが、どちらも元の素直な音に及ばなかった
  （c9c75c1 に残してある）。仕組みは `INSTRUMENTS` と `renderInstruments` のまま残してあるので、
  `index.html` の `#instrument-group` から `hidden` を外せば戻る。
