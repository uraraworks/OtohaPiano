...
| 弾く | 画面下の鍵盤。マルチタッチ（和音・連打）対応。指を滑らせるとグリッサンドになる。鍵盤の広さは 3 段階、オクターブは左右ボタンで移動。左端はドではなくファ始まり（アンパンマンピアノなど子供向けピアノに合わせた）。**音色はピアノ 1 種類のみ**（楽器の選択は当面出していない。理由は下の「設計上の判断」を参照） |
