...
- **鍵盤の音はサンプル音源ではなく Web Audio の合成音**（[src/core/synth.ts](src/core/synth.ts)）。
  ピアノらしさは**倍音ごとに減衰の速さを変える**ことで作っている。実際の弦は高い倍音ほど速く
  減衰するので、全部を同じ速さで減らすと硬い音がいつまでも残ってオルガンのように聞こえる。
  あわせて、倍音の位置を整数倍からわずかに上へずらし（弦の硬さ）、高い音ほど早く減衰させ、
  打鍵の瞬間に短い雑音（ハンマーの当たり）を混ぜている。
