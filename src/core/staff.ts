...
//
// 音部記号は鍵盤の左端に合わせて選ぶ。鍵盤の F3 表示と同じ考え方で、
// どちらの記号でも加線が 2 本を超えないようにするため。
...
/**
 * 鍵盤の左端から、どちらの音部記号を使うか決める。
 * ドリルの出題は左端の音から 1 オクターブ内に収まるので、
 * この選び方なら加線は最大 2 本で済む。
 * しきい値は F3(53)。左端の初期位置がここになったので、
 * 60 のままだとヘ音記号が選ばれ、上側の音が五線の外(SVG の枠外)へ出て切れてしまう。
 */
export function clefFor(baseMidi: number): Clef {
  return baseMidi >= 53 ? "treble" : "bass";
}
