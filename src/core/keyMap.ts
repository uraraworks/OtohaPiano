...
// ミとファの間、シとドの間には黒鍵が無いので、上の段もその位置を空ける（R と I を飛ばす）。
//
//   黒鍵     W E   T Y U   O P   @ [
//   白鍵    A S D F G H J K L ; : ]
//
// 右端の 2 つだけは、真上の白鍵との対応を 1 つずらしてある。
// 厳密に合わせると @ が「ミとファの間」に当たって黒鍵が無く、キーが 1 つ死ぬ。
// @ を ファ♯、[ を ソ♯ にすることで、両方とも鳴らせるようにしている。
//
...
const KEY_TO_SEMITONE: Record<string, number> = {
  ...
  // JIS の「@」。ファ と ソ の間。
  BracketLeft: 18,
  // JIS の「[」。ソ と ラ の間。ラ の白鍵は割り当てが無いが、
  // 鍵盤に出ていれば鳴る（出ていなければ鳴らない）。
  BracketRight: 20,
};
